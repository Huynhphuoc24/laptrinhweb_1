...
-- 7. Liệt kê danh sách mua hàng với tổng tiền từng đơn hàng
...
-- 8. Liệt kê mỗi user chỉ lấy đơn hàng có tổng tiền lớn nhất
...
-- 9. Liệt kê mỗi user chỉ lấy đơn hàng có tổng tiền nhỏ nhất
SELECT user_id, user_name, order_id, total_price, total_products FROM (
    SELECT users.user_id, users.user_name, orders.order_id, SUM(products.product_price) AS total_price, COUNT(products.product_id) AS total_products,
...
-- 10. Liệt kê mỗi user chỉ lấy đơn hàng có số sản phẩm nhiều nhất
SELECT user_id, user_name, order_id, total_price, total_products FROM (
    SELECT users.user_id, users.user_name, orders.order_id, SUM(products.product_price) AS total_price, COUNT(products.product_id) AS total_products,
           RANK() OVER (PARTITION BY users.user_id ORDER BY COUNT(products.product_id) DESC) AS rnk
    FROM users
    JOIN orders ON users.user_id = orders.user_id
    JOIN order_details ON orders.order_id = order_details.order_id
    JOIN products ON order_details.product_id = products.product_id
